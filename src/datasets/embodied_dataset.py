...
class EmDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        images = np.stack([d['color_sensor'] for d in data])
        depths = np.stack([d['depth_sensor'] for d in data])
        poses  = [ correct_yz(d['Ext']) for d in data]  
    ...
    def __len__(self):
        ...
        return self.images.shape[0] - 2 * self.frame_interval
    ...
    def __getitem__(self, index):
        ...
        """
        frame_index = index + self.frame_interval # skip index=0 and -1, cuz where no 
        inputs = {}
        ...
        for i in frame_idxs:
            color = Image.fromarray(self.images[frame_index + i]).convert('RGB')
            if do_flip:
        ...
        depth_gt = self.depths[frame_index]
        ...
        if self.load_pose:
            for i in frame_idxs[1:]:
                ...
                inputs[("cam_T_cam_inv", 0, i)] = torch.from_numpy(cam1_to_cam0(self.poses[frame_index], self.poses[frame_index+i]))

            if len(self.frame_idxs) > 3: # hack: for video input
                poses = [ self.poses[frame_index + f] for f in self.frame_idxs]
                poses_ref_in_other = np.stack([(np.matmul(np.linalg.pinv(pose), poses[0])) for pose in poses[3:]]) # hack: skip 0 -i i 
