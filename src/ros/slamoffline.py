# Testing the orbslam2 python binding
import os
import numpy as np
import cv2
import sys
...
def main(vocab_path, settings_path, sequence_path, association_path):
    ...
    for idx in tqdm(range(num_images)):
        ...
        depth_image = np.load(os.path.join(sequence_path, depth_filenames[idx])) / 1000.
    ...
    save_path = Path(association_path).parent.absolute()
    save_trajectory(slam.get_trajectory_points(), os.path.join(save_path, 'new_trajectory.txt'))
